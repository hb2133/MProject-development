...
public:
	/**
	 * Starts listening for messaging events.
	*/
	UFUNCTION(BlueprintCallable, Category = "Firebase|Messaging", meta = (BlueprintInternalUseOnly = "true"))
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FFBDMMessagingToken, const FString&, Token, EFirebaseMessagingError, Error);


UCLASS()
class UFBMessagingTokenProxy final : public UBlueprintAsyncActionBase
{
	GENERATED_BODY()
public:
	/**
	...
	*/
	UPROPERTY(BlueprintAssignable)
	FFBDMMessagingToken Done;

	/**
	 * An error occurred.
	*/
	UPROPERTY(BlueprintAssignable)
	FFBDMMessagingToken Failed;

public:
	/// This creates a Firebase Installations ID, if one does not exist, and
	/// sends information about the application and the device where it's running to
	/// the Firebase backend.
	UFUNCTION(BlueprintCallable, Category = "Firebase|Messaging", meta = (BlueprintInternalUseOnly = "true"))
	static UFBMessagingTokenProxy* GetToken()
	{
	...
	{
		FFirebaseMessagingLibrary::GetToken(FFirebaseMessagingTokenCallback::CreateUObject(this, &ThisClass::OnTaskOver));
	}

private:

	void OnTaskOver(FString Token, EFirebaseMessagingError Error)
	{
		(Error == EFirebaseMessagingError::None ? Done : Failed).Broadcast(Token, Error);
		SetReadyToDestroy();
	}
