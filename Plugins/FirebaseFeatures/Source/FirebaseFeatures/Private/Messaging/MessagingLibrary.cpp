...
FFirebaseNotification& FFirebaseNotification::operator=(const FFirebaseNotification& other) 
{
	Title = other.Title;
	Body = other.Body;
	Icon = other.Icon;
...
void FFirebaseMessagingLibrary::GetToken(FFirebaseMessagingTokenCallback Callback)
{
#if WITH_FIREBASE_MESSAGING
	firebase::messaging::GetToken().OnCompletion([
		Callback = MoveTemp(Callback)
	](const firebase::Future<std::string> Future) mutable
	{
		FString Token;
		const EFirebaseMessagingError Error = (EFirebaseMessagingError)Future.error();
		if (Error != EFirebaseMessagingError::None)
		{
			UE_LOG(LogFirebaseMessaging, Error, TEXT("Failed to get token: %s"), UTF8_TO_TCHAR(Future.error_message()));
		}
		else if (Future.result())
		{ 
			UE_LOG(LogFirebaseMessaging, Log, TEXT("Got Firebase Installation ID token: %s"), UTF8_TO_TCHAR(Future.result()->c_str()));
			Token = UTF8_TO_TCHAR(Future.result()->c_str());
		}

		AsyncTask(ENamedThreads::GameThread, [Error, Callback = MoveTemp(Callback), Token]() mutable
		{
			Callback.ExecuteIfBound(MoveTemp(Token), Error);
		});
	});
#endif
}
