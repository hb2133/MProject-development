...
public:
	...
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Firebase|Auth", Meta = (DisplayName = "Current User (Firebase)", Keywords = "Get User Firebase Current"))
	...
	UFUNCTION(BlueprintCallable, BlueprintPure, Category = "Firebase|Auth", Meta = (BlueprintAutoCast, CompactNodeTitle = "->"))
	static FCredential FromAppleSignCredential(FAppleSignInCredential Credential);
};
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FDynMulAppleSignInNativeCred, const FAppleSignInCredential&, Credential, int32, Error, const FString&, ErrorMessage);
...
public:
	UPROPERTY(BlueprintAssignable)
	FDynMulAppleSignInNativeCred OnSuccess;

	UPROPERTY(BlueprintAssignable)
	FDynMulAppleSignInNativeCred OnError;

public:

	/// Generates a credential from Apple Sign-In
	/// @param Nonce The nonce used during the sign-in process.
	UFUNCTION(BlueprintCallable, Category = "Firebase|Auth|Apple", meta = (BlueprintInternalUseOnly = "true", DisplayName = "Get Native Credential from Apple Sign-In"))
	static UAppleSignInGetNativeCred* GetCredential(FString Nonce);

	virtual void Activate() override;

private:
	void OnTaskOver(bool bSuccess, FAppleSignInCredential Credential, int32 ErrorCode, FString ErrorMessage);
