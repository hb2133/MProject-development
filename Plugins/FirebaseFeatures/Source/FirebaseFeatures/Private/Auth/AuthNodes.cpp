...
FCredential UAuthMiscLibrary::FromAppleSignCredential(FAppleSignInCredential Credential)
{
	return Credential;
}
...
UAppleSignInGetNativeCred* UAppleSignInGetNativeCred::GetCredential(FString Nonce)
{
	ThisClass* const Proxy = NewObject<ThisClass>();
...
void UAppleSignInGetNativeCred::Activate()
{
	UCredentialLibrary::GetCredentialFromAppleSignIn(MoveTemp(Nonce), 
		FGetNativeAppleSignInCredentialCallback::CreateUObject(this, &ThisClass::OnTaskOver));
}

void UAppleSignInGetNativeCred::OnTaskOver(bool bSuccess, FAppleSignInCredential Credential, int32 ErrorCode, FString ErrorMessage)
{
	(bSuccess ? OnSuccess : OnError).Broadcast(Credential, ErrorCode, ErrorMessage);
	SetReadyToDestroy();
}
