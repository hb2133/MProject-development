...
void UFirebaseAnalyticsLibrary::LogEventWithParameters(const FString& EventName, const TMap<FString, FFirebaseVariant>& Parameters)
{
	...
	// Two loops because array elems can be reallocated on insertion, causing the pointer
	// returned by c_str() to become invalid.
	{ 
		for (const auto& Parameter : Parameters)
		{
			Strings.Emplace(TCHAR_TO_UTF8(*Parameter.Key));
		}

		int32 i = 0;
		for (const auto& Parameter : Parameters)
		{
			RawParameters[i] = firebase::analytics::Parameter(Strings[i].c_str(), Parameter.Value.GetRawVariant());
			++i;
		}
	}
...
void UFirebaseAnalyticsLibrary::GetAnalyticsInstanceId(const FFirebaseAnalyticsStringCallback& Callback)
{
	...
	.OnCompletion([Callback](const firebase::Future<std::string>& _Future) -> void
	{
		...
		if (Error)
		{
			UE_LOG(LogFirebaseAnalytics, Error, TEXT("Failed to get analytics instance ID. Reason: %s"), UTF8_TO_TCHAR(_Future.error_message()));
		}
