...
{
	// Note: Firebase C++ SDK calls abort() on invalid parameters.
	// If you get a crash, DocumentPath is invalid.
	...
	if (!DocumentPath.Contains(TEXT("/")))
	{
		UE_LOG(LogFirestore, Error, TEXT("DocumentPath is invalid."));
		return nullptr;
	}

	if (DocumentPath.EndsWith(TEXT("/")))
	{
		UE_LOG(LogFirestore, Error, TEXT("DocumentPath can't end with '/'."));
		return nullptr;
	}
	...
	Document->Reference = MakeShared<firebase::firestore::DocumentReference>(Firestore->Document(TCHAR_TO_UTF8(*DocumentPath)));

	if (!Document->Reference || !Document->Reference->is_valid())
	{
		UE_LOG(LogFirestore, Error, TEXT("Failed to get document reference."));
		return nullptr;
	}
...
UFirestoreQuery* UFirestore::CollectionGroup(const FString& CollectionId)
{
	...
	check(Firestore);
	*Query->Reference = Firestore->CollectionGroup(TCHAR_TO_UTF8(*CollectionId));
#endif // WITH_FIREBASE_FIRESTORE
