...
UFirestoreDocumentReference::UFirestoreDocumentReference(FVTableHelper& Helper) 
	: Super(Helper)
{
...
{
#if WITH_FIREBASE_FIRESTORE 
	return Reference ? UTF8_TO_TCHAR(Reference->id().c_str()) : FString{};
#else 
...
{
#if WITH_FIREBASE_FIRESTORE 
	return Reference ? UTF8_TO_TCHAR(Reference->path().c_str()) : FString{};
#else 
	return TEXT("FIRESTORE_DISABLED");
#endif // WITH_FIREBASE_FIRESTORE 
}
...
{
#if WITH_FIREBASE_FIRESTORE && defined(FIREBASE_USE_STD_FUNCTION)
	Reference->AddSnapshotListener([Copy = Reference, Callback = MoveTemp(Callback)](
		const firebase::firestore::DocumentSnapshot & Snapshot, 
		firebase::firestore::Error NativeError, 
		const std::string&) mutable -> void
	{
		if (Callback.IsBound())
		{
			const EFirestoreError Error = (EFirestoreError)NativeError;
...
#else // not WITH_FIREBASE_FIRESTORE || not defined(FIREBASE_USE_STD_FUNCTION)
	UE_LOG(LogFirestore, Warning, TEXT("Function AddSnapshotListener is unavailable for this platform."));
#endif
}
