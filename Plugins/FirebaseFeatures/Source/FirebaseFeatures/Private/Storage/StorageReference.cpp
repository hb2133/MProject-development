		...
		.OnCompletion([Callback](const firebase::Future<firebase::storage::Metadata>& Future) -> void
	{
		...
		if (Callback.IsBound())
		{
			...
			if (Future.result())
			{
				...
				{
					Metadata.Metadata = MakeUnique<firebase::storage::Metadata>();
				}
				*Metadata.Metadata = *Future.result();

				UE_LOG(LogTemp, Error, TEXT("RES: %d"), Future.result()->size_bytes());
				UE_LOG(LogTemp, Error, TEXT("RES2: %d"), Metadata.GetSizeBytes());
			}
...
)
{
		...
		[Callback, Controller = Controller.Controller, Listener](const firebase::Future<firebase::storage::Metadata>& Future) mutable -> void
	{
		...
		else if (Future.result())
		{
			...
			{
				_Metadata.Metadata = MakeUnique<firebase::storage::Metadata>();
			}
