...
USTRUCT(BlueprintType)
struct FIREBASEFEATURES_API FAppleSignInCredential
{
	GENERATED_BODY()
public:
	// A JSON Web Token (JWT) that securely communicates information about the user to the app.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Apple Sign-In")
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Apple Sign-In")
	TArray<FString> AuthorizedScopes;
	...
	// The nonce used when requesting the credential.
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Apple Sign-In")
	FString Nonce;
public:

	// Converts this credential to a Firebase Credential.
	FCredential ToCredential() const;

	FORCEINLINE operator FCredential() const
	{
		return ToCredential();
	}
};
...
DECLARE_DELEGATE_FourParams(FGetNativeAppleSignInCredentialCallback, bool /* bSuccess */, FAppleSignInCredential /* Credential */, int32 /* Error */, FString /* ErrorMessage */);
...
public:
	...
	static void GetCredentialFromAppleSignIn(FString Nonce, FGetNativeAppleSignInCredentialCallback Callback);
